...
<div class="mt-4 overflow-x-auto rounded-lg border border-slate-200" tabindex="0" aria-label="Tabel review rotasi">
    <table class="w-full min-w-[600px] text-left text-sm">
        ...
        <tbody>@foreach($schedule['rounds'] as $round)@foreach($round['courts'] as $court)<tr class="border-t border-slate-100"><td class="p-3">{{ $round['number'] }}</td><td class="p-3">{{ $court['number'] }}</td><td class="p-3">{{ collect($court['team_a'])->map(fn ($id) => $players[$id]['name'])->implode(' + ') }}</td><td class="p-3">{{ collect($court['team_b'])->map(fn ($id) => $players[$id]['name'])->implode(' + ') }}</td><td class="p-3 text-slate-500">{{ $loop->first ? (collect($round['rest'])->map(fn ($id) => $players[$id]['name'])->implode(', ') ?: '—') : '—' }}</td></tr>@endforeach@endforeach</tbody>
    </table>
</div>
