...
<div class="shell">
    <aside id="sidebar" aria-label="Navigasi utama">
        ...
        <nav>
            <a @class(['active' => request()->routeIs('dashboard')]) href="{{ route('dashboard') }}"><span aria-hidden="true">⌂</span> Dashboard</a>
            @if(auth()->user()->isAdmin())
                <p>KOMUNITAS</p>
                <a @class(['active' => request()->routeIs('members.*')]) href="{{ route('members.index') }}"><span aria-hidden="true">◎</span> Member</a>
                <a @class(['active' => request()->routeIs('play-sessions.*')]) href="{{ route('play-sessions.index') }}"><span aria-hidden="true">◫</span> Sesi Main</a>
