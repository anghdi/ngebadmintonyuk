...
@section('content')
<div class="page-head"><div><span class="eyebrow">INVENTORI</span><h1>Shuttlecock</h1><p>Catat stok per buah agar pemakaian tiap sesi tetap akurat.</p></div></div>

<div class="inventory-summary">
    @forelse($items as $item)
        <article @class(['low-stock' => (int) $item->stock <= $item->minimum_stock])><span>🏸</span><div><small>{{ $item->brand ?: 'Tanpa merek' }}</small><strong>{{ $item->name }}</strong><p>{{ (int) $item->stock }} buah · {{ number_format((int) $item->stock / $item->pieces_per_tube, 1, ',', '.') }} tabung</p></div><b>{{ (int) $item->stock <= $item->minimum_stock ? 'Stok menipis' : 'Aman' }}</b></article>
    @empty
        <div class="empty-state"><span>🏸</span><h2>Belum ada jenis shuttlecock</h2><p>Tambahkan produk pertama dari formulir di bawah.</p></div>
    @endforelse
</div>

<div class="admin-split inventory-forms">
    <section class="card package-form-card">
        <span class="eyebrow">MASTER BARANG</span><h2>Tambah jenis shuttlecock</h2>
    ...
    <section class="card package-form-card">
        <span class="eyebrow">MUTASI STOK</span><h2>Catat masuk atau terpakai</h2>
        <form method="post" action="{{ route('inventory.movements.store') }}" class="compact-form">
            @csrf
            <label>Shuttlecock<select name="shuttlecock_item_id" required><option value="">Pilih produk</option>@foreach($items as $item)<option value="{{ $item->id }}" @selected(old('shuttlecock_item_id') == $item->id)>{{ $item->brand }} {{ $item->name }} · stok {{ (int) $item->stock }}</option>@endforeach</select></label>
